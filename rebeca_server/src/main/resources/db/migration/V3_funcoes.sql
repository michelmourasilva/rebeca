  ...
  /
create or replace procedure REBECA.prc_recupera_configuracao_serv(p_projeto in varchar2, p_modulo in varchar2, p_id_filtro in numeric, p_criterio_pesquisa in varchar2, p_json_string out clob) is
/*
...
BEGIN
    ...
    select 'select * from ' || configuracao.no_proprietario_banco || '.' || configuracao.no_objeto_banco,
    ...
    if p_id_filtro is not null then
        ...
        select v_nome_ref_cursor || ' WHERE ' || filtro.NO_ATRIBUTO || tipofiltro.DS_TIPO_FILTRO
        into v_nome_ref_cursor
        from REBECA.tb_filtro_servico filtro
                 inner join REBECA.VW_TIPO_FILTRO tipofiltro on filtro.TP_CONDICAO = tipofiltro.ID_TIPO_FILTRO
        where filtro.id_configuracao_servico = v_id_configuracao
          and filtro.id_filtro_servico = v_id_filtro;
    end if;
    -- Abre um cursor
    ...
    dbms_sql.describe_columns2(v_numero_cursor -- Número do ID do cursor para as colunas que estão sendo descritas
        ...
        , v_tabela_descricao_colunas);
    -- Tabela retornada com a descrição de cada uma das colunas da consulta, indexada de 1 ao número de elementos na lista de seleção da consulta
        ...
        loop
            case
                when v_tabela_descricao_colunas(i).col_type in (2, 8) then -- se a coluna for do tipo numerica
                    v_coluna_conteudo := '{"nomeatributo":"' ||
            ...
            dbms_lob.writeappend(v_cabecalho_metadados -- localizador do lob interno a ser gravado.
                ...
                , v_coluna_conteudo);
            -- buffer de entrada para a gravação
    ...
    WHILE DBMS_SQL.FETCH_ROWS(v_numero_cursor) > 0
        LOOP
