create or replace procedure corp.prc_recupera_configuracao_serv(p_sistema in varchar2, p_modulo in varchar2, p_id_filtro in numeric, p_criterio_pesquisa in varchar2, p_json_string out clob) is       
/*
Procedimento armazenado que recupera uma configuração feita para um serviço rest. 
Parâmetros de entrada:
			p_sistema = Nome do sistema que consta na tabela corp.tb_sistema. O código deste sistema deverá está na configuração da tabela corp.tb_configuracao_servico
			p_modulo = Nome do modulo que identifica o objeto do banco através da URI. Este nome deverá está na configuração da tabela corp.tb_configuracao_servico
			p_id_filtro = Número do fitro que está mapeado na tabela corp.tb_filtro_servico
			p_criterio_pesquisa = Valores que servirão de bind para o critério configurado na tabela corp.tb_filtro_servico. Devem seguir a ordem que consta no filtro. Ex: para o bind das variáveis :1 e :2 deve existir dois valores correspondentes a ordem das variáveis. 
Parâmetros de saida:
...
declare 
		v_output clob;
	begin
		ccorp.prc_recupera_configuracao_serv('CENSO_SUPERIOR', 'IES', 1, 'ESCOLA1,ESCOLA2', v_output);
...
BEGIN
        ...
        from corp.tb_configuracao_servico configuracao 
        inner join corp.vw_projeto projeto on configuracao.co_projeto = projeto.co_projeto
        ...
        if p_id_filtro is not null then      
          ...
          select v_nome_ref_cursor || ' WHERE ' || cd_condicao into v_nome_ref_cursor from corp.tb_filtro_servico filtro where filtro.id_configuracao_servico = v_id_configuracao and filtro.id_filtro_servico = v_id_filtro and filtro.cd_condicao <> '@COMPLETO';        
        end if;
		-- Abre um cursor
		...
		if p_id_filtro is not null then
		  if length(v_id_filtro) >= 1 and instr(v_nome_ref_cursor,'corp.fnc_string_virgula_tabela') = 0  then            
			 for i in ( select column_value from (TABLE(corp.fnc_string_virgula_tabela(v_criterio_pesquisa))))
			 loop
	...
	END;
	/
	CREATE OR REPLACE TYPE corp.tp_string_virgula_tabela  AS  TABLE OF VARCHAR2(100);
	/	
	CREATE OR REPLACE    FUNCTION corp.fnc_string_virgula_tabela( p_lista IN VARCHAR2)
      RETURN corp.tp_string_virgula_tabela
	  /*
	  Função que irá transformar uma string com virgula em uma type
	  Ex: SELECT * FROM TABLE(corp.fnc_string_virgula_tabela('valor1, valor2, valor3, valor4, valor5, valor6'))
	  */
    AS
      v_lista VARCHAR2(32767) := p_lista || ',';
      v_virgula_indice PLS_INTEGER;
      v_indice PLS_INTEGER := 1;
      v_tabela_resultado corp.tp_string_virgula_tabela     := corp.tp_string_virgula_tabela();
    BEGIN
      LOOP
  ...
  /
	...
	grant execute on corp.prc_recupera_configuracao_serv to rl_corp_app;
	grant execute on corp.fnc_string_virgula_tabela to rl_corp_app;
	grant execute on corp.tp_string_virgula_tabela to rl_corp_app;